...
class BertTrainer:
    ...
    def __init__(
        ...
        self.optimizer = torch.optim.Adam(
            model.parameters(), lr=learning_rate, weight_decay=0.015
        )
    ...
    def train(self, epoch: int):
        ...
        for i, value in enumerate(self.loader):
            ...
            loss_token = self.ml_criterion(
                token.transpose(1, 2), token_target
            ...
            if index % self._print_every == 0:
                ...
                s = self.training_summary(
                    elapsed, index, average_nsp_loss, average_mlm_loss
                )

                if index % self._accuracy_every == 0:
                    s += self.accuracy_summary(
                        index, token, nsp, token_target, nsp_target, inverse_token_mask
                    )
    ...
    def training_summary(self, elapsed, index, average_nsp_loss, average_mlm_loss):
        ...
        s += (
            f" | Epoch {self.current_epoch + 1} | {index} / {self._batched_len} ({passed}%) | "
    ...
    def accuracy_summary(
        self, index, token, nsp, token_target, nsp_target, inverse_token_mask
    ):
        global_step = self.current_epoch * len(self.loader) + index
